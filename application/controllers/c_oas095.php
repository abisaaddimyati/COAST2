...
class C_OAS095 extends MY_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('m_oas095', 'list_pr_model');		
	}
	function load_view()
	{
	...
	function download()
	{
		...
		$filename="Paid Purchase Request ";
		$search = $this->_get_post_data();
		$employeeID = $this->user['id'];
	...
	private function get_report_header()
	{
		...
		$header['B2'] = "Purchase Request Paid";
...
public function paid_selected(){
		...
		$update = $this->list_pr_model->paid_selected();
  if($update){
